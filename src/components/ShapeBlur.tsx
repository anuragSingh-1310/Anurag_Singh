...
void main() {
    ...
    // Accent color: match the primary portfolio theme color (e.g., golden orange / amber)
    vec3 color = vec3(0.93, 0.49, 0.19); // #ec7d31 / Amber accent
    float alpha = sdf;
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (isMobileOrTablet) {
        ...
        const speed = 0.8;
        vMouse.set(
          cx + Math.cos(time * speed) * radius,
          cy + Math.sin(time * speed) * radius
        );
      }

      // Damp the mouse position for a super smooth drift feel
